...
const VerifyEmail = () => {
  ...
  const from = location.state?.from?.pathname || '/auth/reset-password/change';
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, setSubmitting }) => {
      ...
      try {
        navigate(from, {
          replace: true,
          state: { confirmationCode: values.confirmationCode, username: username }
        });
      } catch (error) {
        console.error(error);
      ...
      >
        <div className="text-center mb-2">
          ...
          <div className="text-2sm text-center text-gray-700">
            Enter OTP sent to &nbsp;
            <p className="text-2sm text-gray-900 font-medium hover:text-primary-active">aa</p>
          </div>
        </div>
